...
impl InspectorUi<'_, '_> {
    ...
    fn ui_for_reflect_map(
        ...
        egui::Grid::new(id).show(ui, |ui| {
            // Reflected maps are unordered, so we need to clone and collect all keys first.
            // We can't have both an immutable borrow of the map's key,
            // and mutably borrow the map to delete or access the element.
            let keys = map
                .iter()
                .map(|it| it.0.to_dynamic())
                .collect::<Vec<_>>();
